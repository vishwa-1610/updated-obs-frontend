...
// --- MAIN IDAHO FORM ---
const IdahoTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    // Personal
    first_name: '', last_name: '', ssn: '', address: '', city: '', state: 'ID', zipcode: '', email: '',
    
    // Status
    status_letter: 'A', // A, B, or C
    ...
    // Line 2 Variables
    additional_withholding: '', // Manual override
    use_multiple_jobs: false,
    multiple_jobs_count: 0,
    other_income: '',
    itemized_line4: '',
    remaining_pay_periods: 52,
  ...
  const statusOptions = [
    { value: "A", label: "A - Single or Head of Household" },
    { value: "B", label: "B - Married (Spouse NOT Working)" },
    { value: "C", label: "C - Married (Both Work or Single Rate)" },
  ];
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          const finalData = { 
              ...formData,
              children_under_17: cleanNumber(formData.children_under_17),
              additional_withholding: cleanNumber(formData.additional_withholding),
              multiple_jobs_count: cleanNumber(formData.multiple_jobs_count),
              other_income: cleanNumber(formData.other_income),
              itemized_line4: cleanNumber(formData.itemized_line4),
              remaining_pay_periods: cleanNumber(formData.remaining_pay_periods),
              signature_image: sig 
          };
    ...
    <>
      ...
      <div className="bg-white px-4 py-2 text-left relative">
        <div className="mb-10 text-center">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Idaho Withholding Certificate</h2>
          <p className="text-gray-500 text-sm mt-1">Form ID W-4</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3"><label className={labelClass}>State</label><input type="text" name="state" value="ID" readOnly className={`${inputClass} bg-gray-50 text-gray-500 cursor-not-allowed text-center`} /></div>
          ...
          {/* WITHHOLDING STATUS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><FileText className="text-blue-600" size={24} /> Withholding Status</h3>
            <div className="grid grid-cols-1 gap-6">
                <div>
                    <label className={labelClass}>Select Filing Status</label>
                    <div className="relative">
                        <select name="status_letter" value={formData.status_letter} onChange={handleChange} className={selectClass}>
          ...
          {/* CALCULATIONS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={24} /> Allowances & Adjustments</h3>
            
            <div className="space-y-6">
                {/* Standard Inputs */}
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className={labelClass}>Children Age 16 or Under</label>
                        <input type="number" name="children_under_17" value={formData.children_under_17} onChange={handleChange} min="0" className={inputClass} />
                    </div>
                    <div>
                    ...
                    <div className="md:col-span-2">
                        <label className={`${checkboxCardClass} ${formData.head_of_household ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                            ...
                            <span className="font-bold text-gray-900">Filing as Head of Household?</span>
                        </label>
                    </div>
                </div>

                {/* Multiple Jobs Worksheet */}
                <label className={`${checkboxCardClass} ${formData.use_multiple_jobs ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="use_multiple_jobs" checked={formData.use_multiple_jobs} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <div>
                        <span className="block font-bold text-gray-900">Use Multiple Jobs Worksheet (Page 2)</span>
                        <span className="text-sm text-gray-500">Check this if you have more than one job or your spouse works.</span>
                    </div>
                </label>

                {formData.use_multiple_jobs && (
                    <div className="bg-white border border-gray-200 rounded-xl p-6 grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-2">
                        <div><label className={labelClass}>Count of Other Jobs</label><input type="number" name="multiple_jobs_count" value={formData.multiple_jobs_count} onChange={handleChange} className={inputClass} /></div>
                        <div><label className={labelClass}>Est. Income from Other Jobs ($)</label><input type="number" name="other_income" value={formData.other_income} onChange={handleChange} className={inputClass} /></div>
                        <div><label className={labelClass}>Line 4 from Itemized Deductions ($)</label><input type="number" name="itemized_line4" value={formData.itemized_line4} onChange={handleChange} className={inputClass} /></div>
                        <div><label className={labelClass}>Remaining Pay Periods (Default 52)</label><input type="number" name="remaining_pay_periods" value={formData.remaining_pay_periods} onChange={handleChange} className={inputClass} /></div>
                    </div>
                )}

                {/* Exempt / Non-Resident */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                    <label className={`${checkboxCardClass} ${formData.exempt ? 'border-red-500 bg-red-50/10' : 'bg-white border-gray-200'}`}>
                        <input type="checkbox" name="exempt" checked={formData.exempt} onChange={handleChange} className="w-5 h-5 text-red-600 rounded" />
                        <span className="font-bold text-gray-900">Claim Exempt from Withholding</span>
                    </label>

                    <label className={`${checkboxCardClass} ${formData.non_resident_alien ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                        <input type="checkbox" name="non_resident_alien" checked={formData.non_resident_alien} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                        <span className="font-bold text-gray-900">Nonresident Alien?</span>
                    </label>
                </div>

                {formData.non_resident_alien && (
                    <div className="animate-in fade-in slide-in-from-top-2">
                        <label className={labelClass}>Select Your Pay Period</label>
                        <div className="relative">
                            <select name="pay_period" value={formData.pay_period} onChange={handleChange} className={selectClass}>
                                {payPeriodOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                            </select>
                            <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                        </div>
                    </div>
