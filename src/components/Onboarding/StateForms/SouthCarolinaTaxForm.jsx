...
// --- MAIN SOUTH CAROLINA FORM ---
const SouthCarolinaTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Filing Status
    filing_status: '1', // 1=Single, 2=Married, 3=Married Separate
    last_name_diff: false,

    // Allowances Worksheet (A-G)
    pa_a: true,  // Yourself (1)
    pa_c: false, // 65 or Blind?
    pa_d: false, // Head of Household?
    pa_e: 0,     // Dependents
    pa_f: 0,     // Deductions (allowance points)

    // Deductions Worksheet (Complex Calc)
    use_deductions: false,
    ded_1: 0, // Est. Itemized
    ded_2: 0, // Est. Standard
    ded_4: 0, // Adjustments
    ded_6: 0, // Non-wage income

    // Withholding
    additional_withholding: '', // Line 6

    // Exemption
    exempt: false,
    exempt_type: '1', // 1=Regular, 2=Military Spouse
    domicile_state: '',
  ...
  const handleChange = (e) => {
    ...
    // --- FIX: Force Zip Code to max 5 digits ---
    if (name === 'zipcode') {
  ...
  // --- CALCULATIONS ---
  // Allowances (Line G)
  const a = formData.pa_a ? 1 : 0;
  const b = formData.filing_status === '2' ? 1 : 0; // Spouse
  const c = formData.pa_c ? 1 : 0;
  const d = formData.pa_d ? 1 : 0;
  const eVal = parseInt(formData.pa_e || 0);
  const fVal = parseInt(formData.pa_f || 0);
  const basicAllowances = a + b + c + d + eVal + fVal;

  // Deductions Worksheet Logic
  let additionalAllowances = 0;
  if (formData.use_deductions) {
      const ded1 = parseFloat(formData.ded_1 || 0);
      const ded2 = parseFloat(formData.ded_2 || 0);
      const ded3 = Math.max(ded1 - ded2, 0);
      const ded4 = parseFloat(formData.ded_4 || 0);
      const ded5 = ded3 + ded4;
      const ded6 = parseFloat(formData.ded_6 || 0);
      const ded7 = ded5 - ded6;
      const ded8 = Math.floor(ded7 / 5050); // Floor division
      const ded9 = basicAllowances;
      additionalAllowances = Math.max(ded8 + ded9, 0);
  }

  // Total Line 5
  const totalAllowances = formData.use_deductions ? additionalAllowances : basicAllowances;
  ...
  return (
    <div className="bg-white px-4 py-2 text-left">
      <div className="mb-10 text-center">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">South Carolina Withholding</h2>
        <p className="text-gray-500 text-sm mt-1">Form SC W-4</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        <section>
            ...
            <div className="space-y-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer p-2 rounded hover:bg-gray-50">
                    <input type="checkbox" name="last_name_diff" checked={formData.last_name_diff} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-sm text-gray-700">Check if your last name differs from that on your Social Security card.</span>
                </label>
            </div>
        </section>
        ...
        <section>
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 space-y-4">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="pa_a" checked={formData.pa_a} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-900 font-medium">A. Enter "1" for yourself</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="pa_c" checked={formData.pa_c} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-900 font-medium">C. Enter "1" if you are 65+ or blind</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="pa_d" checked={formData.pa_d} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-900 font-medium">D. Enter "1" if Head of Household</span>
                </label>
                
                <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                      <label className="text-sm font-semibold text-gray-800">E. Dependents (Number)</label>
                      <input type="number" name="pa_e" value={formData.pa_e} onChange={handleChange} min="0" className="w-20 px-3 py-2 border rounded text-center" />
                </div>

                <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                      <label className="text-sm font-semibold text-gray-800">F. Deduction Allowances ($2k+ expenses)</label>
                      <input type="number" name="pa_f" value={formData.pa_f} onChange={handleChange} min="0" className="w-20 px-3 py-2 border rounded text-center" />
                </div>

                {/* Deductions Worksheet Toggle */}
                <div className="border border-blue-200 bg-white rounded-lg p-4 mt-2">
                    <label className="flex items-center gap-2 mb-4 cursor-pointer">
                        <input type="checkbox" name="use_deductions" checked={formData.use_deductions} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                        <span className="font-bold text-blue-900">Use Itemized Deductions Worksheet?</span>
                    </label>

                    {formData.use_deductions && (
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4 border-l-4 border-blue-400 animate-in fade-in">
                             <div>
                                <label className={labelClass}>Est. Itemized ($)</label>
                                <input type="number" name="ded_1" value={formData.ded_1} onChange={handleChange} className={`${inputClass}`} />
                             </div>
                             <div>
                                <label className={labelClass}>Est. Adjustments ($)</label>
                                <input type="number" name="ded_4" value={formData.ded_4} onChange={handleChange} className={`${inputClass}`} />
                             </div>
                             <div>
                                <label className={labelClass}>Standard Deduction ($)</label>
                                <input type="number" name="ded_2" value={formData.ded_2} onChange={handleChange} className={`${inputClass}`} />
                             </div>
                             <div>
                                <label className={labelClass}>Non-Wage Income ($)</label>
                                <input type="number" name="ded_6" value={formData.ded_6} onChange={handleChange} className={`${inputClass}`} />
                             </div>
                          </div>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-white rounded border border-blue-200 mt-4">
                    <span className="font-bold text-blue-900">Line 5: Total Allowances</span>
        ...
        {/* 4. ADDITIONAL & EXEMPT */}
        <section>
            <h3 className={sectionHeader}><DollarSign className="text-blue-600" size={22} /> Additional & Exemptions</h3>
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-6">
                ...
                <div className="border-t border-gray-200 pt-6">
                    <label className={`${checkboxCardClass} ${formData.exempt ? 'border-red-500 ring-1 ring-red-500 bg-red-50/20' : ''}`}>
                        ...
                        <div>
                             <span className="block font-bold text-gray-900">Check to Claim Exemption</span>
                             <span className="text-xs text-gray-500">I incurred no tax liability last year and expect none this year.</span>
                        </div>
                    </label>

                    {formData.exempt && (
                        <div className="mt-4 pl-8 space-y-3 animate-in fade-in">
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="exempt_type" value="1" checked={formData.exempt_type === '1'} onChange={handleChange} className="text-blue-600" />
                                <span>Standard Exemption</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="exempt_type" value="2" checked={formData.exempt_type === '2'} onChange={handleChange} className="text-blue-600" />
                                <span>Military Spouse (MSRRA)</span>
                            </label>
                            {formData.exempt_type === '2' && (
                                <input type="text" name="domicile_state" value={formData.domicile_state || ''} onChange={handleChange} className={inputClass} placeholder="State of Domicile" />
                            )}
                        </div>
                    )}
                </div>
...
export default SouthCarolinaTaxForm;