...
import { Shield, User, DollarSign, Calendar, ChevronDown, Eraser, Save, PenTool, FileText, Calculator, CheckCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
// --- MAIN MASSACHUSETTS FORM ---
const MassachusettsTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Boxes
    head_of_household: false, // Box A
    blind_self: false,        // Box B
    blind_spouse: false,      // Box C
    exempt: false,            // Box D (Student)

    confirmation_date: new Date().toISOString().split('T')[0],
    signature_image: null
  });

  // 2. INITIALIZE (Safe Load)
  useEffect(() => {
  ...
  const handleSignatureEnd = () => {
    if (sigCanvasRef.current && !sigCanvasRef.current.isEmpty()) {
        ...
        setFormData(prev => ({ ...prev, signature_image: sigData }));
    }
  };
  ...
  // --- CALCULATIONS ---
  const line1 = 1 + (formData.age_65 ? 1 : 0);
  
  let line2 = 0;
  if (formData.claim_spouse) {
      line2 = formData.spouse_65 ? 5 : 4;
  }

  const line3 = parseInt(formData.dependents || 0) + (formData.has_dependent_under_12 ? 1 : 0);
  ...
  return (
    <div className="bg-white px-4 py-2 text-left">
      <div className="mb-10 text-center">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Massachusetts Withholding</h2>
        <p className="text-gray-500 text-sm mt-1">Form M-4 Employee's Withholding Exemption Certificate</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        {/* 2. EXEMPTIONS */}
        <section>
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={22}/> Withholding Exemptions</h3>
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 space-y-6">
                
                {/* Line 1 */}
                <div>
                    <h4 className="font-bold text-gray-800 mb-2">1. Personal Exemption</h4>
                    <label className={`${checkboxCardClass} ${formData.age_65 ? activeCheckboxClass : ''}`}>
                        <input type="checkbox" name="age_65" checked={formData.age_65} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                        <div>
                            <span className="block font-bold text-gray-900">I am age 65 or over</span>
                            <span className="text-xs text-gray-500">Base exemption is 1. Checking this adds +1 (Total 2).</span>
                        </div>
                    </label>
                    <div className="mt-2 text-right text-sm font-bold text-blue-800">Line 1 Total: {line1}</div>
                </div>

                {/* Line 2 */}
                <div>
                    <h4 className="font-bold text-gray-800 mb-2">2. Spouse Exemption</h4>
                    <div className="space-y-3">
                        <label className={`${checkboxCardClass} ${formData.claim_spouse ? activeCheckboxClass : ''}`}>
                            <input type="checkbox" name="claim_spouse" checked={formData.claim_spouse} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                            <div>
                                <span className="block font-bold text-gray-900">Claim Spouse Exemption</span>
                                <span className="text-xs text-gray-500">I am married and claiming exemption for spouse.</span>
                            </div>
                        </label>
                        
                        {formData.claim_spouse && (
                            <label className={`${checkboxCardClass} ml-8 ${formData.spouse_65 ? activeCheckboxClass : ''}`}>
                                <input type="checkbox" name="spouse_65" checked={formData.spouse_65} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                                <div>
                                    <span className="block font-bold text-gray-900">Spouse is 65 or over</span>
                                    <span className="text-xs text-gray-500">Adds additional +1 exemption.</span>
                                </div>
                            </label>
                        )}
                    </div>
                    <div className="mt-2 text-right text-sm font-bold text-blue-800">Line 2 Total: {line2}</div>
                </div>

                {/* Line 3 */}
                <div>
                    <h4 className="font-bold text-gray-800 mb-2">3. Dependents</h4>
                    <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200 mb-3">
                        <label className="text-sm font-semibold text-gray-800">Number of qualified dependents</label>
                        <input type="number" name="dependents" value={formData.dependents || 0} onChange={handleChange} min="0" className="w-20 px-3 py-2 border rounded text-center" />
                    </div>
                    <label className={`${checkboxCardClass} ${formData.has_dependent_under_12 ? activeCheckboxClass : ''}`}>
                        <input type="checkbox" name="has_dependent_under_12" checked={formData.has_dependent_under_12} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                        <div>
                            <span className="block font-bold text-gray-900">Dependent Under 12</span>
                            <span className="text-xs text-gray-500">One or more dependents will be under age 12 at year end (Adds +1).</span>
                        </div>
                    </label>
                    <div className="mt-2 text-right text-sm font-bold text-blue-800">Line 3 Total: {line3}</div>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-white rounded border border-blue-200 mt-4">
                    <div className="flex items-center gap-2">
                         ...
                         <label className="font-bold text-blue-900">Line 4: Total Exemptions</label>
                    </div>
                    <span className="font-bold text-xl text-blue-900 bg-white px-4 py-1 rounded border border-blue-200">{line4}</span>
                </div>
            </div>
        </section>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-200 pt-4">
                    <label className={checkboxCardClass}>
                        <input type="checkbox" name="head_of_household" checked={formData.head_of_household} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                        <span className="font-medium text-gray-900">A. Head of Household</span>
                    </label>
                    <label className={checkboxCardClass}>
                        <input type="checkbox" name="blind_self" checked={formData.blind_self} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                        <span className="font-medium text-gray-900">B. I am Blind</span>
                    </label>
                    <label className={checkboxCardClass}>
                        <input type="checkbox" name="blind_spouse" checked={formData.blind_spouse} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                        <span className="font-medium text-gray-900">C. Spouse is Blind</span>
                    </label>
                    <label className={`${checkboxCardClass} ${formData.exempt ? 'border-red-500 bg-red-50/20' : ''}`}>
                        ...
                        <span className="font-medium text-gray-900">D. Student Exempt (Income &lt; $8,000)</span>
                    </label>
                </div>
            </div>
        </section>
...
export default MassachusettsTaxForm;