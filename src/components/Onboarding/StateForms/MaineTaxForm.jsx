...
// --- MAIN MAINE FORM ---
const MaineTaxForm = ({ initialData, onSubmit }) => {
  ...
  // --- STYLES DEFINED HERE (AT THE TOP) ---
  ...
  const [formData, setFormData] = useState({
    ...
    ssn: '', // STANDARD NAME
    address: '', city: '', state: 'ME', zipcode: '', email: '',
    ...
    // Additional / Exempt
    additional_withholding: '', // STANDARD NAME
    exempt: false,
    exempt_type: '', // STANDARD NAME (a, b, c, d, e)
  ...
  const maritalOptions = [
    { value: "1", label: "1 - Single or Head of Household" },
    { value: "2", label: "2 - Married Filing Jointly" },
    { value: "3", label: "3 - Married (Withhold at Single Rate)" }
  ];

  const exemptOptions = [
    { value: "a", label: "a - Exempt on federal W-4" },
    { value: "b", label: "b - Exempt on federal W-4P" },
    { value: "c", label: "c - Resident, no liability" },
    { value: "d", label: "d - Retirement payments, no liability" },
    { value: "e", label: "e - Military spouse (MSRRA)" }
  ];
  ...
  const clearSignature = () => { sigCanvasRef.current.clear(); setFormData(prev => ({ ...prev, signature_image: null })); };
  ...
  const handleSubmit = async (e) => {
      ...
      if (!sig) { setErrorState({ isOpen: true, title: "Missing Signature", message: "Please sign the form in the 'Digital Signature' box before submitting." }); return; }
      
      // Check Standard SSN
      if (!formData.first_name || !formData.last_name || !formData.ssn) { setErrorState({ isOpen: true, title: "Missing Information", message: "Please ensure your Name and SSN are filled out." }); return; }
      ...
      try {
          ...
          setIsSubmitting(false);
          setErrorState({ isOpen: true, title: "Submission Failed", message: error.response?.data?.error || error.message || "Something went wrong." });
      }
  };
    ...
    <>
      ...
      <div className="bg-white px-4 py-2 text-left relative">
        <div className="mb-10 text-center">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Maine Withholding Certificate</h2>
          <p className="text-gray-500 text-sm mt-1">Form W-4ME</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3"><label className={labelClass}>State</label><input type="text" name="state" value="ME" readOnly className={`${inputClass} bg-gray-50 text-gray-500 cursor-not-allowed text-center`} /></div>
          ...
          {/* 2. MARITAL STATUS & WORKSHEET */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={24} /> Allowances Worksheet</h3>
            
            <div className="space-y-6">
                <div>
                    <label className={labelClass}>Marital Status (Box 3)</label>
                    <div className="relative">
                        <select name="marital_status_code" value={formData.marital_status_code} onChange={handleChange} className={selectClass}>
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 grid grid-cols-1 gap-4">
                    <label className={`${checkboxCardClass} ${formData.me_line_a ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                        <input type="checkbox" name="me_line_a" checked={formData.me_line_a} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                        <span className="font-bold text-gray-900">Line A: No one else can claim you?</span>
                    </label>
                    <label className={`${checkboxCardClass} ${formData.me_line_b ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                        <input type="checkbox" name="me_line_b" checked={formData.me_line_b} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                        <span className="font-bold text-gray-900">Line B: Claim spouse? (Married Filing Jointly)</span>
                    </label>
                    <label className={`${checkboxCardClass} ${formData.me_line_c ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                        <input type="checkbox" name="me_line_c" checked={formData.me_line_c} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                        <span className="font-bold text-gray-900">Line C: Head of Household?</span>
                    </label>
                    <div><label className={labelClass}>Line D: Number of Children/Dependents</label><input type="number" name="me_line_d" value={formData.me_line_d} onChange={handleChange} min="0" className={inputClass} /></div>
                </div>

                <div className="pt-2"><label className={labelClass}>Additional Withholding (Line 5) ($)</label><div className="relative max-w-sm"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 font-bold">$</span><input type="number" name="additional_withholding" value={formData.additional_withholding} onChange={handleChange} className={`${inputClass} pl-8`} placeholder="0.00" /></div></div>
            </div>
          </section>
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <label className={`${checkboxCardClass} ${formData.exempt ? 'border-green-500 ring-1 ring-green-500 bg-green-50/10' : 'bg-white border-gray-200'}`}>
                ...
                <div><span className="block font-bold text-gray-900">Claim Exempt?</span><span className="text-sm text-gray-500">I claim exemption from withholding.</span></div>
            </label>
            {formData.exempt && (
                <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                    <label className={labelClass}>Exemption Type</label>
                    <div className="relative">
                        <select name="exempt_type" value={formData.exempt_type} onChange={handleChange} className={selectClass}>
                            <option value="">-- Select Type --</option>
                            {exemptOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                        </select>
                        <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    </div>
                </div>
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="w-full">
                <div className="flex justify-between items-center mb-3"><label className={labelClass}>Digital Signature <span className="text-red-500">*</span></label><button type="button" onClick={clearSignature} className="text-xs font-bold text-red-500 hover:text-red-700 flex items-center gap-1 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-colors"><Eraser size={14}/> Clear Signature</button></div>
