...
import { Shield, User, DollarSign, Calendar, ChevronLeft, ChevronRight, ChevronDown, Eraser, Save, PenTool, FileText, Calculator, Flag, HeartPulse } from 'lucide-react';
...
// --- MAIN IOWA FORM ---
const IowaTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Filing Status
    filing_status_ia: '1', // 1=Single, 2=HOH, 3=Married
    spouse_income: false, // For Married status

    // Allowances (Values are in Dollars $)
    personal_allow: 40, // Default $40
    depend_allow: 0,
    itemized_allow: 0,
    adjust_allow: 0,
    childcare_allow: 0,
    ...
    // New Hire Reporting (Page 4 specifics)
    date_of_birth: '',
    start_date: new Date().toISOString().split('T')[0],
    health_coverage: false,
    coverage_date: '',
  ...
  const filingStatuses = [
    { code: '1', label: 'Single / Married Filing Separately' },
    { code: '2', label: 'Head of Household' },
    { code: '3', label: 'Married Filing Jointly' }
  ];
  ...
  const totalAllowances = 
    parseInt(formData.personal_allow || 0) + 
    parseInt(formData.depend_allow || 0) + 
    parseInt(formData.itemized_allow || 0) + 
    parseInt(formData.adjust_allow || 0) + 
    parseInt(formData.childcare_allow || 0);
  ...
  return (
    <div className="bg-white px-4 py-2 text-left">
      <div className="mb-10 text-center">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Iowa W-4 & New Hire Reporting</h2>
        <p className="text-gray-500 text-sm mt-1">Centralized Employee Registry Reporting Form</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        <section>
            <h3 className={sectionHeader}><FileText className="text-blue-600" size={22} /> Filing Status & Exemptions</h3>
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 mb-6">
                ...
                <div className="relative mb-4">
                    <select name="filing_status_ia" value={formData.filing_status_ia} onChange={handleChange} className={`${inputClass} appearance-none cursor-pointer pr-10`}>
                ...
                {/* Married Sub-Question */}
                {formData.filing_status_ia === '3' && (
                    <div className="flex items-center gap-4 bg-white p-3 rounded border border-blue-200 animate-in fade-in">
                        <span className="text-sm font-bold text-gray-700">Does your spouse have income?</span>
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="spouse_income" checked={formData.spouse_income === true} onChange={() => setFormData(p => ({...p, spouse_income: true}))} className="text-blue-600" /> Yes
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="spouse_income" checked={formData.spouse_income === false} onChange={() => setFormData(p => ({...p, spouse_income: false}))} className="text-blue-600" /> No
                            </label>
                        </div>
                    </div>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className={`${checkboxCardClass} ${formData.exempt ? 'border-red-500 ring-1 ring-red-500 bg-red-50/20' : ''}`}>
                    ...
                    <div>
                         <span className="block font-bold text-gray-900">Claim Exempt</span>
                         <span className="text-xs text-gray-500">I incurred no tax liability last year and expect none this year.</span>
                    </div>
                </label>

                <div className={`p-4 border rounded-lg transition-all ${formData.military_spouse ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    ...
                    {formData.military_spouse && (
                        <input type="text" name="domicile_state" value={formData.domicile_state} onChange={handleChange} className={`${inputClass} mt-1`} placeholder="Enter Domicile State" />
                    )}
                </div>
            </div>
        </section>

        {/* 3. ALLOWANCES (DOLLAR AMOUNTS) */}
        {!formData.exempt && (
        <section>
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={22} /> Allowances (In Dollars $)</h3>
            
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 space-y-4">
                 <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                    <label className="text-sm font-medium text-gray-700">1. Personal Allowances (Standard $40)</label>
                    <div className="relative w-32"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="personal_allow" value={formData.personal_allow} onChange={handleChange} className={`${inputClass} pl-6 py-1`} /></div>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                    <label className="text-sm font-medium text-gray-700">2. Dependent Allowances</label>
                    <div className="relative w-32"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="depend_allow" value={formData.depend_allow} onChange={handleChange} className={`${inputClass} pl-6 py-1`} /></div>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                    <label className="text-sm font-medium text-gray-700">3. Itemized Allowances</label>
                    <div className="relative w-32"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="itemized_allow" value={formData.itemized_allow} onChange={handleChange} className={`${inputClass} pl-6 py-1`} /></div>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                    <label className="text-sm font-medium text-gray-700">4. Adjustment Allowances</label>
                    <div className="relative w-32"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="adjust_allow" value={formData.adjust_allow} onChange={handleChange} className={`${inputClass} pl-6 py-1`} /></div>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded border border-gray-200">
                    <label className="text-sm font-medium text-gray-700">5. Child/Dependent Care Credit</label>
                    <div className="relative w-32"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="childcare_allow" value={formData.childcare_allow} onChange={handleChange} className={`${inputClass} pl-6 py-1`} /></div>
                 </div>

                 {/* Total Line 6 */}
                 <div className="flex items-center justify-between p-3 bg-blue-100 rounded border border-blue-200 mt-4">
                    <label className="font-bold text-blue-900">6. Total Allowances (Sum of 1-5)</label>
                    <span className="font-bold text-xl text-blue-900">${totalAllowances}</span>
                 </div>

                 {/* Additional Withholding Line 7 */}
                 <div className="pt-4 mt-2 border-t border-gray-300">
                     <label className={labelClass}>7. Additional Withholding Amount ($)</label>
                     <div className="relative max-w-sm"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500">$</span><input type="number" name="additional_withholding" value={formData.additional_withholding} onChange={handleChange} className={`${inputClass} pl-8`} /></div>
                 </div>
            </div>
        </section>
        )}
        ...
        <section>
             <h3 className={sectionHeader}><Flag className="text-blue-600" size={22} /> New Hire Reporting Details</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-xl border border-gray-200">
                 <div>
                     <CustomDatePicker label="Date of Birth" name="date_of_birth" value={formData.date_of_birth} onChange={handleChange} placeholder="YYYY-MM-DD" />
                 </div>
                 <div>
                     <CustomDatePicker label="Start Date (Date of Hire)" name="start_date" value={formData.start_date} onChange={handleChange} />
                 </div>
                 
                 <div className="md:col-span-2 bg-white p-4 rounded border border-gray-200">
                     <div className="flex items-center justify-between mb-2">
                        <label className="font-semibold text-gray-800 flex items-center gap-2"><HeartPulse size={18} className="text-red-500"/> Is health insurance available?</label>
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="health_coverage" checked={formData.health_coverage === true} onChange={() => setFormData(p => ({...p, health_coverage: true}))} /> Yes</label>
                            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="health_coverage" checked={formData.health_coverage === false} onChange={() => setFormData(p => ({...p, health_coverage: false}))} /> No</label>
                        </div>
                     </div>
                     {formData.health_coverage && (
                         <div className="animate-in fade-in slide-in-from-top-1 mt-3">
                             <CustomDatePicker label="Date Qualified for Coverage" name="coverage_date" value={formData.coverage_date} onChange={handleChange} />
                         </div>
                     )}
                 </div>
             </div>
