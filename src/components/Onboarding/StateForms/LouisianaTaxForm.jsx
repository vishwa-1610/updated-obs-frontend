...
// --- MAIN LOUISIANA FORM ---
const LouisianaTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    address: '', city: '', state: 'LA', zipcode: '', email: '',

    // L-4 Specific
    l4_personal_exemptions: '1', // Block A: "0", "1", or "2"
    dependents: 0,               // Block B
    additional_withholding: '',  // Line 8
    l4_filing_status: '2',       // Line 3: "1", "2", or "3"
  ...
  const blockAOptions = [
    { value: "0", label: "0 - Neither self nor spouse" },
    { value: "1", label: "1 - Self only" },
    { value: "2", label: "2 - Self + Spouse" }
  ];

  const filingStatusOptions = [
    { value: "1", label: "No exemptions/dependents (1)" },
    { value: "2", label: "Single (2)" },
    { value: "3", label: "Married (3)" }
  ];
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          // Map strictly to Serializer fields
          const finalData = { 
              ...formData,
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3"><label className={labelClass}>State</label><input type="text" name="state" value="LA" readOnly className={`${inputClass} bg-gray-50 text-gray-500 cursor-not-allowed text-center`} /></div>
          ...
          {/* 2. EXEMPTIONS & STATUS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={24} /> Exemptions & Status</h3>
            
            <div className="space-y-6">
                
                {/* Block A */}
                <div>
                    <label className={labelClass}>Block A: Personal Exemptions (Line 6)</label>
                    <div className="relative">
                        <select name="l4_personal_exemptions" value={formData.l4_personal_exemptions} onChange={handleChange} className={selectClass}>
                            {blockAOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                        </select>
                        <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                    </div>
                </div>
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6">
                    <label className={labelClass}>Block B: Number of Dependents (Line 7)</label>
                    <input type="number" name="dependents" value={formData.dependents} onChange={handleChange} min="0" className={inputClass} />
                    <p className="text-xs text-gray-500 mt-2">Enter number of dependents (not self or spouse).</p>
                </div>

                {/* Line 8 */}
                <div className="pt-2">
                    <label className={labelClass}>Additional Withholding ($) (Line 8)</label>
                ...
                <div className="pt-4 border-t border-gray-100">
                    <label className={labelClass}>Filing Status (Line 3)</label>
                    <div className="relative">
                        <select name="l4_filing_status" value={formData.l4_filing_status} onChange={handleChange} className={selectClass}>
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <p className="text-sm text-gray-700 mb-6 font-medium bg-gray-50 p-4 rounded-lg">
                I declare under the penalties for filing false reports that this certificate has been examined by me and to the best of my knowledge and belief is true, correct, and complete.
            </p>
...
export default LouisianaTaxForm;