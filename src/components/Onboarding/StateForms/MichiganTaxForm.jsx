...
import { Shield, User, DollarSign, Calendar, ChevronDown, Eraser, Save, PenTool, FileText, Calculator, Car, Flag,ChevronLeft ,ChevronRight, ChevronDownCircle } from 'lucide-react';
...
// --- MAIN MICHIGAN FORM ---
const MichiganTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    last_name: '',
    ssn: '',
    dob: '', // Date of Birth (Required for MI)
    driver_license: '', // Optional but on form
    address: '',
    city: '',
    state: 'MI',
    zipcode: '',
    ...
    // New Employee Check
    new_employee: false, // "Are you a new employee?"
    hire_date: '',       // If yes, enter date of hire
    ...
    exempt: false,
    exempt_type: '', // 'a', 'b', 'c'
    exempt_text: '', // Explanation text for 'b' or 'c'
  ...
  const handleExemptTypeChange = (type) => {
      setFormData(prev => ({
          ...prev,
          exempt_type: type,
          exempt_text: '' // Reset text when type changes
      }));
  };
  ...
  return (
    <div className="bg-white px-4 py-2 text-left">
      <div className="mb-10 text-center">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Michigan Withholding Certificate</h2>
        <p className="text-gray-500 text-sm mt-1">Form MI-W4</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        ...
        <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-6"><label className={labelClass}>Date of Birth</label><CustomDatePicker name="dob" value={formData.dob} onChange={handleChange} placeholder="YYYY-MM-DD" /></div>

                <div className="md:col-span-6"><label className={labelClass}>Driver's License No.</label><div className="relative"><input type="text" name="driver_license" value={formData.driver_license} onChange={handleChange} className={`${inputClass} pl-10`} placeholder="Optional" /><Car size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400"/></div></div>
        ...
        {/* 2. NEW EMPLOYEE CHECK */}
        <section>
            <h3 className={sectionHeader}><Flag className="text-blue-600" size={22} /> Employment Status</h3>
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 flex items-center justify-between flex-wrap gap-4">
                <div className="flex items-center gap-4">
                    <span className="font-semibold text-gray-700">Are you a new employee?</span>
                    <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="new_employee" checked={formData.new_employee === true} onChange={() => setFormData(p => ({...p, new_employee: true}))} className="text-blue-600" /> Yes</label>
                    <label className="flex items-center gap-2 cursor-pointer"><input type="radio" name="new_employee" checked={formData.new_employee === false} onChange={() => setFormData(p => ({...p, new_employee: false}))} className="text-blue-600" /> No</label>
                </div>
                {formData.new_employee && (
                    <div className="w-full md:w-auto animate-in fade-in slide-in-from-top-2">
                        <CustomDatePicker label="Date of Hire" name="hire_date" value={formData.hire_date} onChange={handleChange} />
                    </div>
                )}
            </div>
        </section>

        {/* 3. EXEMPTIONS & WITHHOLDING */}
        <section>
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={22} /> Exemptions & Withholding</h3>
            <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 space-y-6">
                
                {/* Line 6: Exemptions */}
                <div>
                    <label className={labelClass}>6. Total number of personal and dependent exemptions</label>
                    <input type="number" name="exemptions" value={formData.exemptions} onChange={handleChange} min="0" className={inputClass} />
                    <p className="text-xs text-gray-500 mt-1">See instructions if you claim more than your personal exemption.</p>
                </div>

                {/* Line 7: Additional */}
                <div>
                    <label className={labelClass}>7. Additional amount to withhold ($)</label>
                ...
                <div className="border-t border-blue-200 pt-6">
                    <label className="flex items-center gap-3 cursor-pointer mb-4">
                        ...
                        <span className="font-bold text-lg text-blue-900">8. Claim Exemption from Withholding</span>
                    </label>
                    ...
                    {formData.exempt && (
                        <div className="space-y-4 pl-8 animate-in fade-in slide-in-from-top-2">
                            ...
                            {/* Option A */}
                            <label className={`${checkboxCardClass} ${formData.exempt_type === 'a' ? activeCheckboxClass : ''}`}>
                                <input type="radio" name="exempt_type" checked={formData.exempt_type === 'a'} onChange={() => handleExemptTypeChange('a')} className="mt-1 text-blue-600" />
                                <div>
                                    <span className="block font-bold text-gray-900">a. Renaissance Zone</span>
                                    <span className="text-xs text-gray-500">I reside in a Renaissance Zone.</span>
                                </div>
                            </label>

                            {/* Option B */}
                            <label className={`${checkboxCardClass} ${formData.exempt_type === 'b' ? activeCheckboxClass : ''}`}>
                                <input type="radio" name="exempt_type" checked={formData.exempt_type === 'b'} onChange={() => handleExemptTypeChange('b')} className="mt-1 text-blue-600" />
                                <div className="w-full">
                                    <span className="block font-bold text-gray-900">b. Treaty Exempt</span>
                                    <span className="text-xs text-gray-500 block mb-2">Member of a Native American tribe residing in designated area.</span>
                                    {formData.exempt_type === 'b' && (
                                        <input type="text" name="exempt_text" value={formData.exempt_text} onChange={handleChange} className={inputClass} placeholder="Enter Tribe Name" />
                                    )}
                                </div>
                            </label>

                            {/* Option C */}
                            <label className={`${checkboxCardClass} ${formData.exempt_type === 'c' ? activeCheckboxClass : ''}`}>
                                <input type="radio" name="exempt_type" checked={formData.exempt_type === 'c'} onChange={() => handleExemptTypeChange('c')} className="mt-1 text-blue-600" />
                                <div className="w-full">
                                    <span className="block font-bold text-gray-900">c. Military Spouse</span>
                                    <span className="text-xs text-gray-500 block mb-2">My state of domicile is not Michigan.</span>
                                    {formData.exempt_type === 'c' && (
                                        <input type="text" name="exempt_text" value={formData.exempt_text} onChange={handleChange} className={inputClass} placeholder="Enter State of Domicile" />
                                    )}
                                </div>
                            </label>
        ...
        <section className="bg-gray-50 rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-700 mb-6 font-medium">
                Under penalties of perjury, I certify that the number of exemptions and/or the indicated tax treatment for the purposes of withholding is correct.
            </p>
...
export default MichiganTaxForm;