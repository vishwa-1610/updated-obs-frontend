...
import { User, Shield, DollarSign, Calendar, ChevronDown, Eraser, Save, PenTool, FileText, Calculator, Building2, Loader2, XCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
// --- MAIN OHIO FORM ---
const OhioTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    dependent_on_other: false, // Line 1
    single_or_mfs: true,       // Line 2 (Default True -> 0 on line 2, False -> 1 on line 2)
    dependents: 0,             // Line 3
    extra_withholding: '',     // Line 5

    // Section III - Waivers
    oh_reciprocity: false,
    oh_resident_military: false,
    oh_nonresident_military: false,
    oh_nonresident_spouse: false,
    oh_statutory_exempt: false,
  ...
  const handleChange = (e) => {
    ...
    if (name === 'zipcode') { setFormData(prev => ({ ...prev, [name]: value.replace(/\D/g, '').slice(0, 5) })); return; }
    if (name === 'district_number') { setFormData(prev => ({ ...prev, [name]: value.slice(0, 4) })); return; }
  ...
  const handleSubmit = async (e) => {
      ...
      try {
          ...
          const finalData = { 
              ...
              extra_withholding: cleanNumber(formData.extra_withholding),
              signature_image: sig 
          };
  ...
  const checkboxCardClass = "flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition-all hover:border-blue-400 bg-white border-gray-200 hover:shadow-sm";
    ...
    <>
      ...
      <div className="bg-white px-4 py-2 text-left relative">
        <div className="mb-10 text-center">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Ohio Withholding Certificate</h2>
          <p className="text-gray-500 text-sm mt-1">Form IT 4</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3"><label className={labelClass}>State</label><input type="text" name="state" value="OH" readOnly className={`${inputClass} bg-gray-50 text-gray-500 cursor-not-allowed text-center`} /></div>
                ...
                <div className="md:col-span-8"><label className={labelClass}>School District of Residence</label><input type="text" name="school_district" value={formData.school_district} onChange={handleChange} className={inputClass} placeholder="e.g. Columbus City" /></div>
                <div className="md:col-span-4"><label className={labelClass}>District No.</label><input type="text" name="district_number" value={formData.district_number} onChange={handleChange} className={inputClass} placeholder="4 Digits" maxLength={4} /></div>
            </div>
          </section>

          {/* 2. EXEMPTIONS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Calculator className="text-blue-600" size={24} /> Section II: Exemptions</h3>
            
            <div className="space-y-4">
                <label className={`${checkboxCardClass} ${formData.dependent_on_other ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="dependent_on_other" checked={formData.dependent_on_other} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                    <div>
                        <span className="block font-bold text-gray-900">Line 1: Dependent on another?</span>
                        <span className="text-sm text-gray-500">Check if you are claimed as a dependent on another Ohio income tax return.</span>
                    </div>
                </label>

                <label className={`${checkboxCardClass} ${formData.single_or_mfs ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="single_or_mfs" checked={formData.single_or_mfs} onChange={handleChange} className="mt-1 w-5 h-5 text-blue-600 rounded" />
                    <div>
                        <span className="block font-bold text-gray-900">Line 2: Single or Married Filing Separately?</span>
                        <span className="text-sm text-gray-500">Uncheck if you are Married Filing Jointly.</span>
                    </div>
                </label>

                <div className="pt-2">
                    <label className={labelClass}>Line 3: Number of Dependents</label>
                    <input type="number" name="dependents" value={formData.dependents} onChange={handleChange} min="0" className={inputClass} />
                </div>

                <div className="pt-2">
                    <label className={labelClass}>Line 5: Additional Withholding ($)</label>
                    <div className="relative max-w-sm"><span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 font-bold">$</span><input type="number" name="extra_withholding" value={formData.extra_withholding} onChange={handleChange} className={`${inputClass} pl-8`} placeholder="0.00" /></div>
                </div>
            </div>
          </section>
          ...
          {/* 3. WAIVERS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Shield className="text-blue-600" size={24} /> Section III: Withholding Waiver</h3>
            <p className="text-sm text-gray-500 mb-4">Check all that apply to exempt yourself from Ohio withholding.</p>
            
            <div className="space-y-3">
                <label className={`${checkboxCardClass} ${formData.oh_reciprocity ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="oh_reciprocity" checked={formData.oh_reciprocity} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-800 font-medium">Full-year resident of IN, KY, MI, PA, or WV (Reciprocity)</span>
                </label>

                <label className={`${checkboxCardClass} ${formData.oh_resident_military ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="oh_resident_military" checked={formData.oh_resident_military} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-800 font-medium">Resident Military Stationed Outside Ohio</span>
                </label>

                <label className={`${checkboxCardClass} ${formData.oh_nonresident_military ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="oh_nonresident_military" checked={formData.oh_nonresident_military} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-800 font-medium">Non-resident Military Stationed in Ohio</span>
                </label>

                <label className={`${checkboxCardClass} ${formData.oh_nonresident_spouse ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="oh_nonresident_spouse" checked={formData.oh_nonresident_spouse} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-800 font-medium">Non-resident Military Spouse</span>
                </label>

                <label className={`${checkboxCardClass} ${formData.oh_statutory_exempt ? activeCheckboxClass : 'bg-white border-gray-200'}`}>
                    <input type="checkbox" name="oh_statutory_exempt" checked={formData.oh_statutory_exempt} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded" />
                    <span className="text-gray-800 font-medium">Statutory Exemption (R.C. 5747.06)</span>
                </label>
            </div>
          </section>
