...
// --- MAIN ILLINOIS FORM ---
const IllinoisTaxForm = ({ initialData, onSubmit }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    address: '', city: '', state: 'IL', zipcode: '', email: '',
    ...
    <>
      ...
      <div className="bg-white px-4 py-2 text-left relative">
        <div className="mb-10 text-center">
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Illinois Withholding Certificate</h2>
          <p className="text-gray-500 text-sm mt-1">Employee's Illinois Withholding Allowance Certificate (IL-W-4)</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-3"><label className={labelClass}>State</label><input type="text" name="state" value="IL" readOnly className={`${inputClass} bg-gray-50 text-gray-500 cursor-not-allowed text-center`} /></div>
          ...
          {/* 2. EXEMPTION STATUS */}
          <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
            <h3 className={sectionHeader}><Shield className="text-blue-600" size={24} /> Exemption Status</h3>
            ...
            <label className={`${checkboxCardClass} ${formData.exempt ? 'border-green-500 ring-1 ring-green-500 bg-green-50/10' : 'bg-white border-gray-200'}`}>
                ...
                />
                <div>
                    <span className="block font-bold text-gray-900">I am Exempt from Illinois Withholding</span>
                    <span className="text-sm text-gray-500">
                        Check this box if you are exempt from federal and Illinois Income Tax withholding.
                    </span>
                </div>
            </label>
          </section>

          {/* 3. ALLOWANCES (Hidden if Exempt) */}
          <div className={`transition-all duration-300 ${formData.exempt ? 'opacity-40 pointer-events-none grayscale' : 'opacity-100'}`}>
              <section className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-6">
                        <label className={labelClass}>Step 1: Basic Allowances (Dependents)</label>
                        <input type="number" name="dependents" value={formData.dependents} onChange={handleChange} min="0" className={inputClass} />
                        <p className="text-xs text-gray-500 mt-2">Enter the number of dependents (other than yourself or spouse) you will claim.</p>
                    </div>

                    {/* Additional Allowances */}
                    <div className="bg-white border border-gray-200 rounded-xl p-6">
                        <label className={labelClass}>Step 2: Additional Allowances (Age/Blindness)</label>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                            <label className="flex items-center gap-2"><input type="checkbox" name="you_65" checked={formData.you_65} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded"/> You are 65+</label>
                            <label className="flex items-center gap-2"><input type="checkbox" name="you_blind" checked={formData.you_blind} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded"/> You are Blind</label>
                            <label className="flex items-center gap-2"><input type="checkbox" name="sp_65" checked={formData.sp_65} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded"/> Spouse is 65+</label>
                            <label className="flex items-center gap-2"><input type="checkbox" name="sp_blind" checked={formData.sp_blind} onChange={handleChange} className="w-5 h-5 text-blue-600 rounded"/> Spouse is Blind</label>
                        </div>
                    </div>

                    {/* Additional Withholding */}
                    <div className="pt-4 border-t border-gray-100">
                        <label className={labelClass}>Step 3: Additional Withholding ($)</label>
...
export default IllinoisTaxForm;