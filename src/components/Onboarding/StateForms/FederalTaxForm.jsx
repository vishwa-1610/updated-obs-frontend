...
import { 
  ...
  ShieldCheck, Zap, FileJson, Check, Loader2, Coins, Landmark
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden scale-100 animate-in zoom-in-95 duration-300 relative border border-white/50">
        <div className="bg-green-50 p-8 flex flex-col items-center justify-center border-b border-green-100">
          ...
          <h3 className="text-2xl font-bold text-gray-900">W-4 Generated!</h3>
...
const FederalTaxPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    use_step2b: false, 
    mj_higher_annual_wages: '', mj_lower_annual_wages: '', mj_pay_periods: 26,
    
    // --- Step 3 ---
    kids_under_17: 0, other_dependents: 0, other_credits: '',

    // --- Step 4 ---
    step4_other_income: '', step4_deductions: '', step4_extra_withholding: '', 

    // --- Deductions Worksheet ---
    use_deductions_worksheet: false,
    deductions_1a: '', deductions_1b: '', deductions_1c: '',
    deductions_3a: '', deductions_3b: '', // Seniors
    deductions_5: '', // Student Loan
    deductions_6a: '', deductions_6b: '', deductions_6c: '', deductions_6d: '', deductions_6e: '',
    deductions_8a: '', 
    deductions_12: '', // Charity
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      const payload = { employee_id: employeeId, ...formData };
      const response = await axios.post(`${API_BASE_URL}/api/federal-tax/`, payload, { responseType: 'blob', validateStatus: (status) => status < 500 });
  ...
  const handleContinue = () => {
      ...
      navigate(NO_STATE_TAX_STATES.includes(code) ? `/i9?id=${employeeId}` : `/state?id=${employeeId}&state=${code}`);
  };

  useEffect(() => {
      if(employeeId && !formData.first_name) {
          // Pre-fill logic can go here
      }
  }, [employeeId]);
      ...
      <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row min-h-screen">
        ...
        <div className="w-full lg:w-7/12 p-4 md:p-8 lg:p-16 xl:p-24 bg-slate-50 flex flex-col relative z-0">
            <div className="lg:hidden mb-8 text-center mt-4">
                <div className="inline-flex items-center justify-center w-12 h-12 bg-slate-900 rounded-xl mb-4 shadow-lg text-white"><DollarSign size={24} /></div>
            ...
            <div className="max-w-3xl w-full mx-auto relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-8 relative">
                            ...
                            {!formData.multiple_jobs_two && (
                                <div className="bg-blue-50 p-5 rounded-xl border border-blue-100 animate-in fade-in">
                                    <label className="flex items-center gap-3 cursor-pointer mb-4">
                                        ...
                                        <span className="font-bold text-blue-900">Use Multiple Jobs Worksheet?</span>
                                    </label>
                                    {formData.use_step2b && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <InputField label="Higher Paying Job Wages ($)" name="mj_higher_annual_wages" value={formData.mj_higher_annual_wages} onChange={handleChange} type="number" placeholder="0.00" />
                                            <InputField label="Lower Paying Job Wages ($)" name="mj_lower_annual_wages" value={formData.mj_lower_annual_wages} onChange={handleChange} type="number" placeholder="0.00" />
                    ...
                    <div className={CARD_STYLE}>
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center bg-gray-50 p-4 rounded-xl">
                                ...
                                <div className="w-32"><InputField name="kids_under_17" value={formData.kids_under_17} onChange={handleChange} type="number" /></div>
                            </div>
                            <div className="flex justify-between items-center bg-gray-50 p-4 rounded-xl">
                                <label className="text-sm font-bold text-gray-700">Other Dependents (x $500)</label>
                                <div className="w-32"><InputField name="other_dependents" value={formData.other_dependents} onChange={handleChange} type="number" /></div>
                            </div>
                            <div className="flex justify-between items-center bg-gray-50 p-4 rounded-xl">
                                <label className="text-sm font-bold text-gray-700">Other Credits ($)</label>
                                <div className="w-32"><InputField name="other_credits" value={formData.other_credits} onChange={handleChange} type="number" /></div>
                            </div>
                        </div>
                    </div>

                    {/* --- STEP 4: ADJUSTMENTS & DEDUCTIONS WORKSHEET --- */}
                    <div className={CARD_STYLE}>
                        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                        ...
                        <div className="mb-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                            <label className="flex items-center gap-3 cursor-pointer">
                                ...
                                <span className="font-bold text-slate-700">Calculate Deductions using Worksheet (Page 4)?</span>
                            </label>
                            ...
                            {formData.use_deductions_worksheet && (
                                <div className="mt-4 pt-4 border-t border-slate-200 space-y-6 animate-in fade-in">
                                    ...
                                    {/* Section A: Wages & Income */}
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><Coins size={14}/> Wage Details</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <InputField label="1a: Qualified Tips" name="deductions_1a" value={formData.deductions_1a} onChange={handleChange} type="number" />
                                            <InputField label="1b: Overtime Pay" name="deductions_1b" value={formData.deductions_1b} onChange={handleChange} type="number" />
                                            <InputField label="1c: Other Non-Exempt Wages" name="deductions_1c" value={formData.deductions_1c} onChange={handleChange} type="number" />
                                        </div>
                                    </div>

                                    {/* Section B: Seniors (Lines 3a, 3b) - ADDED */}
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><User size={14}/> Seniors (Age 65+)</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <InputField label="3a: Age 65+ (Self) - Enter $6,000" name="deductions_3a" value={formData.deductions_3a} onChange={handleChange} type="number" placeholder="0.00" />
                                            <InputField label="3b: Age 65+ (Spouse) - Enter $6,000" name="deductions_3b" value={formData.deductions_3b} onChange={handleChange} type="number" placeholder="0.00" />
                                        </div>
                                    </div>

                                    {/* Section C: Other Adjustments (Line 5) - ADDED */}
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><Landmark size={14}/> Other Adjustments</h4>
                                        <InputField label="5: Student Loan Interest / Deductible IRA" name="deductions_5" value={formData.deductions_5} onChange={handleChange} type="number" placeholder="0.00" />
                                    </div>

                                    {/* Section D: Itemized Deductions */}
                                    <div>
                                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2"><ListChecks size={14}/> Itemized Deductions (Schedule A)</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <InputField label="6a: Medical (>7.5%)" name="deductions_6a" value={formData.deductions_6a} onChange={handleChange} type="number" />
                                            <InputField label="6b: State/Local Taxes" name="deductions_6b" value={formData.deductions_6b} onChange={handleChange} type="number" />
                                            <InputField label="6c: Mortgage Interest" name="deductions_6c" value={formData.deductions_6c} onChange={handleChange} type="number" />
                                            <InputField label="6d: Gifts to Charity" name="deductions_6d" value={formData.deductions_6d} onChange={handleChange} type="number" />
                                            <InputField label="6e: Other Deductions" name="deductions_6e" value={formData.deductions_6e} onChange={handleChange} type="number" />
                                        </div>
                                    </div>

                                    {/* Section E: Totals & Standard Charity */}
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <InputField label="8a: Total Income (Line 8a)" name="deductions_8a" value={formData.deductions_8a} onChange={handleChange} type="number" />
                                        {/* Line 12 - ADDED */}
                                        <InputField label="12: Cash Gifts to Charity (Standard Deduction)" name="deductions_12" value={formData.deductions_12} onChange={handleChange} type="number" placeholder="Up to $300 ($600 joint)" />
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <InputField label="4(a): Other Income (Not from jobs)" name="step4_other_income" value={formData.step4_other_income} onChange={handleChange} type="number" placeholder="0.00" />
                            <InputField 
                                label={formData.use_deductions_worksheet ? "4(b): Deductions (Auto-Calculated)" : "4(b): Deductions"} 
                    ...
                    <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100 flex items-start gap-4">
                        ...
                        <div>
                            <h4 className="font-bold text-amber-900">Claim Exemption from Withholding</h4>
                            <p className="text-sm text-amber-800/80 mt-1">I certify that I had no federal income tax liability in the previous year and I expect to have no federal income tax liability this year.</p>
                        </div>
                    </div>
                    ...
                    <div className={CARD_STYLE}>
                        ...
                        <div ref={containerRef} className="border-2 border-dashed border-gray-300 rounded-xl h-48 relative bg-gray-50/50 cursor-crosshair hover:bg-gray-50 transition-colors">
