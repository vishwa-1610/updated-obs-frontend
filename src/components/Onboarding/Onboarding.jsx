...
const Onboarding = () => {
  ...
  // --- CRITICAL FIX: SEARCH & FETCH LOGIC ---
  useEffect(() => {
    // 1. Base params (Page & Sort)
    const params = { 
      page: pagination?.currentPage || 1, 
      ordering: ordering,
    };

    // 2. Map Frontend Inputs -> Backend Partial Search (__icontains)
    // This allows "net" to find "Netflix"
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= Math.ceil(pagination.count / 10)) {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="p-6">
        ...
        {/* ✅ UI FIX: Clean Container Style 
           This wrapper handles the border and rounded corners, 
           matching your ClientTable look perfectly.
        */}
